            ...
            .addOnSuccessListener(querySnapshot -> {
                ...
                if (querySnapshot.isEmpty()) {
                    ...
                    for (com.google.firebase.firestore.DocumentSnapshot doc : querySnapshot.getDocuments()) {
                        ...
                        card.setBackgroundColor(android.graphics.Color.parseColor((isRead != null && isRead) ? "#1A2535" : "#1E2D42"));
...
for filepath in FILES:
    ...
    if "showNotificationPanelDialog()" not in content or "private void showNotificationPanelDialog()" not in content:
        # split by last '}'
        parts = content.rsplit("}", 1)
        if len(parts) == 2:
            content = parts[0] + METHOD + "\n"
